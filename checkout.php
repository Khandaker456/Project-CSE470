	   		...
	   		if ($get_product->rowCount()> 0) {
	   			while ($fetch_p = $get_product->fetch(PDO::FETCH_ASSOC)){
	   				$insert_order = $conn->prepare("INSERT INTO `orders`(id, user_id, name, number, email, address, address_type,, $method, $product_id, price, qty) VALUES(?,?,?,?,?,?,?,?,?,?,?)");
	   					$insert_order->execute([unique_id(), $user_id, $name, $number, $email, $address, $address_type, $method, $fetch_p['price'], 1]);
	   					header('location: order.php');

	   			}
	   		}else{
	   			$warning_msg[] = 'something wrong';
	   		}
	   	}elseif ($varify_cart->rowCount()>0) {
	   		while ($f_cart = $varify_cart->fetch(PDO::FETCH_ASSOC)) {
	   			$insert_order = $conn->prepare("INSERT INTO `orders`(id, user_id, name, number, email, address, address_type,, $method, $product_id, price, qty) VALUES(?,?,?,?,?,?,?,?,?,?,?)");
	   					$insert_order->execute([unique_id(), $user_id, $name, $number, $email, $address, $address_type, $method, $f_cart['price'],$f_cart['qty'], 1]);
	   					header('location: order.php');
	   		}
	   		if (insert_order) {
...
<head>
	...
	<title>Gimmick - checkout page</title>
</head>
<body>
	...
	<div class="main">		
		...
		<div class="title2">
			<a href="home.php">checkout summary</a><span>our shop</span>
		</div>
				...
				<div class="row">
					<form method="post">
						...
						<div class="flex">
							<div class="box">
								...
								<div class="input-field">
									...
									<select name="method" class="input">
										...
										<option value="UPI or Upay">UPI or Upay</option>
							...
							<div class="box">
								<div class="input-field">
									<p>address in line 01<span>*</span></p>
									<input type="text" name="name" required maxlength="50" placeholder="e.g flat & building number" class="input">
								</div>
								<div class="input-field">
									<p>address in line 02<span>*</span></p>
									<input type="text" name="street" required maxlength="50" placeholder="street name" class="input">
								</div>
								<div class="input-field">
									<p>city name<span>*</span></p>
									<input type="text" name="flat" required maxlength="50" placeholder="Enter your city name" class="input">
								</div>
								<div class="input-field">
									<p>country name<span>*</span></p>
									<input type="text" name="country" required maxlength="50" placeholder="Your country" class="input">
								</div>
								<div class="input-field">
